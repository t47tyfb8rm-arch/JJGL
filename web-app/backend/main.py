...
async def fetch_fund_from_eastmoney(fund_code: str, stock_index: Optional[IndexInfo] = None, bond_index: Optional[IndexInfo] = None, hs300_index: Optional[IndexInfo] = None, force: int = 0, lite: int = 0) -> Optional[FundInfo]:
    ...
    try:
        ...
        if is_user_holding:
            ...
            total_return = round(realized_yield_pct + yield_pct, 2)
...
async def enhance_funds_with_deepseek(
    ...
    prompt = (
        "你是偏保守的基金组合投顾助手，目标是给出可执行但不夸大的决策参考。"
        "只返回JSON对象，格式：{\"funds\":{\"基金代码\":{\"advice\":\"操作结论+触发条件+风险边界，36到70字\","
        "\"market_env\":\"影响该基金的市场背景，不超过70字\", \"position_advice\":\"当前动作建议，只能是观察/持有/等待买点/小额试探/谨慎补仓/止盈观察之一，并说明触发条件\", "
        "\"risk_tips\":\"最主要风险和失效条件，不超过60字\", \"risk_level\":\"低/中/高\", \"trend\":\"趋势标签，不超过6字\"}}}。"
        "advice必须像决策备忘录，不要只写上涨下跌；必须结合预估收益、买点距离、持仓状态、支撑压力、近7天/1月/半年表现、未来一周重要事件。"
        "如果未接近买点，要明确等待什么信号；如果已持仓，要说明继续持有/补仓/止盈观察的边界。不要给金额建议，不要保证收益。"
    )
    try:
        async with httpx.AsyncClient(timeout=12.0, follow_redirects=True) as client:
...
async def confirm_buy(fund_code: str, payload: Optional[dict] = None):
    """
    用户买入/补仓指定基金。
    - buy_price: 用户确认后的成本净值，默认最新净值
    - buy_date/date: 交易日期，默认最新净值日期
    补仓时不再按份额权重加权，前端带出当前成本，由用户修正后直接覆盖成本。
    """
    if fund_code not in WATCHED_FUNDS:
        raise HTTPException(status_code=404, detail=f"基金 {fund_code} 不在关注列表")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="成本净值必须是有效数字")
    if buy_nav <= 0:
        raise HTTPException(status_code=400, detail="成本净值必须大于0")
...
async def correct_position_cost(fund_code: str, payload: Optional[dict] = None):
    ...
    if cost_nav <= 0:
        raise HTTPException(status_code=400, detail="成本净值必须大于0")
    shares = float(old.get("shares", 1.0) or 1.0)
